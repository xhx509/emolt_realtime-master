        ...
        for i in emolt_df.index:  #valuable_tele_df is the valuable telemetry data during start time and end time 
            if int(emolt_df['vessel_n'][i].split('_')[1])==telemetrystatus_df['Vessel#'][j]:
                ...
                try:    #try to get the gomofs data in the same location
                    gmf_temp,gmf_depth=gom.get_gomofs(dtime=ptime,latp=latpt,lonp=lonpt,depth=depthpt,fortype='tempdepth')
                except KeyboardInterrupt:
                    sys.exit()
                except:
...
def read_emolt_all(path='http://emolt.org/emoltdata/emolt.dat',endtime=datetime.now()):
...
def main():
    ...
    dictionarypath='/var/www/vhosts/emolt.org/httpdocs/emoltdata/dictionary.json'
